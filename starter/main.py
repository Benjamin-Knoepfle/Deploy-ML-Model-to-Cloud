...
class PredictionPayload(BaseModel):
    ...
    # usage of a validator for data_validation might be good.
    # Atm validation is done withing prediction function
    # @validator('name')
...
def make_prediction(features: PredictionPayload) -> Dict[str, str]:
    ...
    if features.age < 0 or features.age > 100:
        raise HTTPException(
            status_code=400,
            detail=f"Age mus be between 0 and 100, got {features.age}")
